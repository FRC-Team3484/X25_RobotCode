...
double SwerveModule::Get() {
    return _drive_motor.Get();
}

void SwerveModule::SetVoltage(volt_t voltage, degree_t direction) {
    _drive_motor.SetVoltage(voltage);
    double steer_output = _steer_pid_controller.Calculate(_GetSteerAngle(), direction);
    _steer_motor.Set(steer_output);
}