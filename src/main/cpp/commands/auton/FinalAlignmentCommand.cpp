...
FinalAlignmentCommand::FinalAlignmentCommand(
	...
	bool far) :
    _drivetrain_subsystem(drivetrain_subsystem), _far{far} {
...
bool FinalAlignmentCommand::IsFinished() { 
	if (_far){
		...
		(_drivetrain_subsystem->GetPose().Translation().Distance(_target_pose.Translation()) < FAR_POSE_TOLERANCE && 
		abs(_drivetrain_subsystem->GetPose().Rotation().Degrees() - _target_pose.Rotation().Degrees()) < FAR_ANGLE_TOLERANCE);
	} else {
		return _counter > FINAL_ALIGN_EXIT ||
